    ...
    try {
        ...
        const filteredData = data
            .filter((item) => {
                ...
                // Ensure the data falls within the requested time bounds
        ...
        // Remove duplicates keeping the most recent forecast for a given startTime
        // The data is sorted by startTime. If there are multiple predictions for the same startTime,
        // we should keep the one with the latest publishTime (most accurate/recent).
        // Or we can just return all of them and let the frontend do a scatter plot. 
        // Let's aggregate to a single line: for each startTime, keep the one with the latest publishTime
        ...
        for (const item of filteredData) {
            ...
            if (!existing) {
                ...
                // If this one was published later than the existing one, replace it
